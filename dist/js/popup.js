        ...
        if (!(3 > w) && (3 != w || this.g && (this.h.h || this.g.oa() || Nb(this.g)))) {
          ...
          if (this.o) {
            ...
            if (this.P) {
              ...
              this.o = this.o && c;
              if (!c) L2(this.i, this.l, g, "[Invalid Chunked Response]"), Q(this), Qb(this);
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  function mostrarErro(mensagem) {
    ...
    if (erroMensagemElement) {
      erroMensagemElement.textContent = mensagem || "N\xE3o foi poss\xEDvel extrair o n\xFAmero de telefone.";
    }
  ...
  function extrairTelefone() {
    console.log("Iniciando extra\xE7\xE3o de telefone...");
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
      ...
      if (!currentTab.url || !currentTab.url.includes("web.whatsapp.com")) {
        mostrarErro("Esta extens\xE3o s\xF3 funciona no WhatsApp Web. Por favor, abra https://web.whatsapp.com");
        return;
      }
      ...
      chrome.tabs.sendMessage(
        ...
        function(response) {
          ...
          if (chrome.runtime.lastError) {
            console.error("Erro ao comunicar com a p\xE1gina:", chrome.runtime.lastError.message);
            mostrarErro("Erro ao comunicar com a p\xE1gina: " + chrome.runtime.lastError.message);
            return;
          }
          if (!response) {
            mostrarErro("N\xE3o foi poss\xEDvel conectar ao WhatsApp Web. Tente recarregar a p\xE1gina.");
            return;
          }
          if (response.success && response.telefone) {
            mostrarResultado(response.telefone);
          } else {
            let mensagemErro = "N\xE3o foi poss\xEDvel extrair o telefone.";
            if (response.error) {
  ...
  async function carregarOrcamentos(telefone) {
    try {
      const orcamentosContato = [];
      const q = query(collection(db, "orcamentos"), where("telefone", "==", telefone));
      const querySnapshot = await getDocs(q);
      querySnapshot.forEach((doc2) => {
        const data = doc2.data();
        orcamentosContato.push({ descricao: data.descricao, valor: data.valor, data: data.data });
      });
      if (orcamentosContato.length === 0) {
        ...
        orcamentosContato.forEach((orcamento) => {
      ...
      console.error("Erro ao carregar or\xE7amentos:", error);
    }
  }
  async function salvarOrcamento() {
    ...
    if (!descricao || !valor) {
      alert("Por favor, preencha a descri\xE7\xE3o e o valor do or\xE7amento.");
      return;
    }
    if (!telefoneAtual) {
      alert("N\xE3o h\xE1 um telefone selecionado.");
      return;
    }
    try {
      ...
      console.error("Erro ao salvar or\xE7amento:", error);
      alert("Erro ao salvar or\xE7amento.");
    }
  }
